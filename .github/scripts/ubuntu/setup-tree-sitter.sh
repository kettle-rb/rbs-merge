...
#   --workspace PATH: Workspace root path (defaults to /workspaces/rbs-merge for GHA compatibility)

SUDO=""
INSTALL_CLI=false
BUILD_FROM_SOURCE=false
WORKSPACE_ROOT="/workspaces/rbs-merge"
...
PACKAGES="build-essential pkg-config wget gcc g++ make zlib1g-dev libssl-dev libreadline-dev libyaml-dev libxml2-dev libxslt1-dev libcurl4-openssl-dev software-properties-common libffi-dev git"
...
echo "Building and installing tree-sitter-rbs..."
...
echo "Downloading tree-sitter-rbs from GitHub..."
if ! wget -q https://github.com/joker1007/tree-sitter-rbs/archive/refs/heads/main.zip -O rbs.zip; then
  echo "ERROR: Failed to download tree-sitter-rbs" >&2
  echo "URL: https://github.com/joker1007/tree-sitter-rbs/archive/refs/heads/main.zip" >&2
  echo "Check your internet connection and GitHub availability" >&2
  exit 1
fi

if ! unzip -q rbs.zip; then
  echo "ERROR: Failed to unzip tree-sitter-rbs" >&2
  exit 1
fi

cd tree-sitter-rbs-main

# Compile parser.c and scanner.c
...
if ! gcc -shared -o libtree-sitter-rbs.so parser.o scanner.o; then
  echo "ERROR: Failed to link tree-sitter-rbs.so" >&2
  exit 1
fi

# Install to system
if ! $SUDO cp libtree-sitter-rbs.so /usr/local/lib/; then
  echo "ERROR: Failed to copy tree-sitter-rbs.so to /usr/local/lib/" >&2
  exit 1
fi
...
echo "Successfully installed tree-sitter-rbs to /usr/local/lib/"
...
echo "  TREE_SITTER_RBS_PATH=/usr/local/lib/libtree-sitter-rbs.so"
